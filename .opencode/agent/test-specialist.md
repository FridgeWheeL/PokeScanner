---
description: Writes and maintains MSTest unit tests using the dotnet-testing skill. Use when writing new tests or fixing existing tests.
mode: subagent
---
...
using MSTest, Moq, and FluentAssertions.
...
- Test file: one test class per source class, in the matching namespace
  under `*.Tests`.
- Use `[TestClass]` on test classes.
- Use `[TestMethod]` for unit tests, `[DataRow]` for
  parameterised.
- AAA structure with blank line separators.
- Mock external dependencies with Moq; never mock the system
  under test.
